...
>
> Note that on POSIX platforms, enabling `terminateProcessTree` places the subprocess tree in a separate process group.
> As such, terminal-generated signals received by the parent process (such as SIGINT generated by Ctrl-C) will no longer be automatically received by the subprocess tree.
