...
`char& String::back(s32 ofs = -1)`
`const char& String::back(s32 ofs = -1) const`
...
`bool String::startsWith(StringView arg) const`
> Returns `true` if the string begins with the specified prefix.

`bool String::endsWith(StringView arg) const`
> Returns `true` if the string ends with the specified suffix.

`s32 String::find(StringView substr, u32 startPos = 0) const`
...
`s32 String::find(const MatchFunc& matchFunc, u32 startPos = 0) const`
...
`s32 String::reverseFind(StringView substr, s32 startPos = -1) const`
...
`Array<StringView> String::split(StringView separator) const;`
> Splits the string at each occurrence of `separator` and returns an array of views.

`String String::join(ArrayView<const StringView> comps) const;`
...
`template <typename... Args> bool String::match(StringView pattern, const Args&&... args)`
...
`MutStringView String::mutStringView() const`
...
`static String String::allocate(u32 numBytes)`
> Allocates a new string of the specified size. The contents are uninitialized.

`static String String::adopt(char* bytes, u32 numBytes)`
...
`static String String::format(StringView fmt, const Args&... args)`
...
`static String String::fromDateTime(const DateTime& dateTime);`
