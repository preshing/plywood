...
`static void* Heap::alloc(uptr numBytes)`
...
`static void* Heap::realloc(void* ptr, uptr numBytes)`
...
`static void* Heap::free(void* ptr)`
> Frees a previously allocated block, returning the memory to the heap.

`static void* Heap::allocAligned(uptr numBytes, u32 alignment)`
...
`template <typename Type> static Type* Heap::create<Type>(Args&&... args)`
...
`template <typename Type> static void Heap::destroy(Type* obj)`
...
`static void Heap::setOutOfMemoryHandler(Functor<void()> handler)`
...
`static Heap::Stats Heap::getStats()`
