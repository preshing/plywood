...
`Float2 Float2::safeNormalized(const Float2& fallback = {1, 0}, float epsilon = 1e-6f) const`
...
`Float3 Float3::safeNormalized(const Float3& fallback = {1, 0, 0}, float epsilon = 1e-9f) const`
...
`static Mat3x4 Mat3x4::fromQuaternion(const Quaternion& q, const Float3& pos = 0)`
...
`static Mat4x4 Mat4x4::fromQuaternion(const Quaternion& q, const Float3& pos = 0)`
