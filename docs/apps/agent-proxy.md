...
It's meant to help avoid leaking API keys when using agents to work on `agent` itself.
...
|---|---|---|
| `-p[=<port>]` | `--port[=<port>]` | TCP port to listen on. Defaults to 8082. |
...
[`agent`](/docs/apps/agent.md) when the `-p/--provider` option is specified. To enable a specific provider, the environment variable named by `apiKeyEnv` must
